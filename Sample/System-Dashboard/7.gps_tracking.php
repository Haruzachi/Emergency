...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $lat = $_POST['latitude'];
    $lng = $_POST['longitude'];
    $icon = $_POST['icon_type'];
    $stmt = $conn->prepare("INSERT INTO gps_tracking (incident_id, latitude, longitude, icon_type) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("idds", $incident_id, $lat, $lng, $icon); $stmt->execute(); $stmt->close();
}

// Fetch data for map markers
$locations = [];
$sql = "SELECT g.latitude, g.longitude, g.icon_type, i.name, i.incident_type
        FROM gps_tracking g 
        JOIN incidents i ON g.incident_id = i.id";
...
<body>
  ...
  <h2>Real-Time GPS Tracking</h2>
    <form method="POST">
        <select name="incident_id" class="form-control mb-2">
            <?php $res = $conn->query("SELECT id, name FROM incidents");
            while ($r = $res->fetch_assoc()) echo "<option value='{$r['id']}'>{$r['name']}</option>"; ?>
        </select>
        <input name="latitude" class="form-control mb-2" placeholder="Latitude" required>
        <input name="longitude" class="form-control mb-2" placeholder="Longitude" required>
        <input name="icon_type" class="form-control mb-2" placeholder="Type (Fire, EMS, Police)" required>
    ...
    <script>
        ...
        var icons = {
            Fire: L.icon({ iconUrl: 'https://maps.google.com/mapfiles/ms/icons/red-dot.png', iconSize: [32, 32] }),
            EMS: L.icon({ iconUrl: 'https://maps.google.com/mapfiles/ms/icons/green-dot.png', iconSize: [32, 32] }),
            Police: L.icon({ iconUrl: 'https://maps.google.com/mapfiles/ms/icons/blue-dot.png', iconSize: [32, 32] }),
        };
        ...
        markers.forEach(function(item) {
            var marker = L.marker([parseFloat(item.latitude), parseFloat(item.longitude)], {
                icon: icons[item.icon_type] || L.icon.default
            }).addTo(map);

            marker.bindPopup("<strong>" + item.name + "</strong><br>Type: " + item.incident_type);
        });
    </script>
