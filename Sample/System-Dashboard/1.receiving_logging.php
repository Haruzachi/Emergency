<?php
$host = "localhost"; $user = "root"; $pass = ""; $db = "emergency_system";
$conn = new mysqli($host, $user, $pass, $db); if ($conn->connect_error) { die("Connection failed: " . $conn->connect_error); }
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $name = $_POST['name']; $type = $_POST['incident_type']; $location = $_POST['location'];
    $lat = $_POST['latitude']; $lng = $_POST['longitude'];
    $stmt = $conn->prepare("INSERT INTO incidents (name, incident_type, location, latitude, longitude) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssdd", $name, $type, $location, $lat, $lng); $stmt->execute(); $stmt->close();
}
?>
...
<body>
		...
		<ul class="side-menu top">
			...
			<li class="emergency-dropdown">
				...
				<ul class="emergency-submenu" id="emergencySubmenu">
					...
					<li>
						<a href="2.prioritization_dispatch.php">
	...
	<section id="content">
	 ...
	 <h2>Emergency Call Logging</h2>
<form method="POST">
  <input class="form-control mb-2" name="name" placeholder="Caller Name" required>
  <input class="form-control mb-2" name="incident_type" placeholder="Incident Type" required>
  <input class="form-control mb-2" name="location" placeholder="Location" required>
  <input class="form-control mb-2" name="latitude" placeholder="Latitude" required>
  <input class="form-control mb-2" name="longitude" placeholder="Longitude" required>
  <button class="btn btn-primary">Submit</button>
</form>
<hr><h4>Logged Incidents</h4>
<table class="table table-bordered"><tr><th>ID</th><th>Name</th><th>Type</th><th>Location</th><th>Status</th></tr>
<?php $result = $conn->query("SELECT * FROM incidents ORDER BY id DESC");
while ($row = $result->fetch_assoc()) {
  echo "<tr><td>{$row['id']}</td><td>{$row['name']}</td><td>{$row['incident_type']}</td><td>{$row['location']}</td><td>{$row['status']}</td></tr>";
} ?>
</table>
