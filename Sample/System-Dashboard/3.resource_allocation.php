...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO resources (incident_id, unit_type, unit_name) VALUES (?, ?, ?)");
    $stmt->bind_param("iss", $incident_id, $unit_type, $unit_name); $stmt->execute(); $stmt->close();
}
?>
...
<form method="POST">
    ...
    <select name="unit_type" class="form-control mb-2">
        <option>EMS</option><option>Fire</option><option>Police</option>
    </select>
    <input type="text" name="unit_name" placeholder="Unit Name" class="form-control mb-2" required>
    <button class="btn btn-primary">Assign</button>
</form>
<hr>
<h4>Allocated Resources</h4>
<table class="table table-bordered">
<tr><th>ID</th><th>Incident</th><th>Type</th><th>Unit</th><th>Status</th></tr>
<?php
$res = $conn->query("SELECT r.*, i.name FROM resources r JOIN incidents i ON r.incident_id = i.id ORDER BY r.id DESC");
while ($r = $res->fetch_assoc()) {
    echo "<tr><td>{$r['id']}</td><td>{$r['name']}</td><td>{$r['unit_type']}</td><td>{$r['unit_name']}</td><td>{$r['status']}</td></tr>";
}
?>
</table>
