...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $received = $_POST['time_received'];
    $arrived = $_POST['time_arrived'];
    $stmt = $conn->prepare("INSERT INTO response_time (incident_id, time_received, time_arrived) VALUES (?, ?, ?)");
    $stmt->bind_param("iss", $incident_id, $received, $arrived); $stmt->execute(); $stmt->close();
}
?>
...
<form method="POST">
    <select name="incident_id" class="form-control mb-2" required>
        <?php $res = $conn->query("SELECT id, name FROM incidents");
        while ($r = $res->fetch_assoc()) echo "<option value='{$r['id']}'>{$r['name']}</option>"; ?>
    </select>
    <input type="datetime-local" name="time_received" class="form-control mb-2" required>
    <input type="datetime-local" name="time_arrived" class="form-control mb-2" required>
    <button class="btn btn-info">Submit</button>
</form>
<hr>
<h4>Response Records</h4>
<table class="table table-bordered"><tr><th>ID</th><th>Incident</th><th>Minutes</th></tr>
<?php
$res = $conn->query("SELECT r.*, i.name FROM response_time r JOIN incidents i ON r.incident_id = i.id");
while ($r = $res->fetch_assoc()) {
    echo "<tr><td>{$r['id']}</td><td>{$r['name']}</td><td>{$r['response_minutes']} mins</td></tr>";
}
?>
